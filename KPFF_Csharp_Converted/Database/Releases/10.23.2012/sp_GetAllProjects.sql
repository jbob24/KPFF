/****** Object:  StoredProcedure [dbo].[sp_GetAllProjects]    Script Date: 10/27/2012 12:14:05 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_GetAllProjects]') AND type in (N'P', N'PC'))
...
SELECT ([ID]) AS ProjectID, ProjectNo, UPPER(ProjectName) AS ProjectName, PICCode, PM1Code 
...
,(select MAX(s.lastmodifieddate) from tblSchedule s where s.ProjectID = e.ID) as LastModifiedDate
