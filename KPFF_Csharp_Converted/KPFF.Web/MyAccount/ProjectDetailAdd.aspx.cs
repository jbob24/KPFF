...
namespace KPFF.PMP.MyAccount
{
    partial class ProjectDetailAdd : System.Web.UI.Page
    {
        ...
        protected int EmployeeID
        {
            get
            {
                _empID = Session["EmployeeID"].GetValueOrDefault<int>(); // basToolbox.Nz(Session["EmployeeID"], 0);
        ...
        private bool ValidateForm()
        {
            ...
            decimal projNo = default(decimal);

            if (string.IsNullOrEmpty(txtProjectNo.Text) | !decimal.TryParse(txtProjectNo.Text, out projNo))
            {
                lblError.Text = "Please enter a valid project number.";
                lblError.Visible = true;
                return false;
            }
