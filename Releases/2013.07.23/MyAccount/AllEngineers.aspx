<%@ Page Language="C#" MasterPageFile="~/MyAccount/_TemplateMaster.master" AutoEventWireup="true" CodeBehind="AllEngineers.aspx.cs" Inherits="KPFF.PMP.MyAccount.AllEngineers" Title="Untitled Page" %>
        ...
        <table cellpadding="0" cellspacing="0" border="0">
            ...
            <tr>
                <td colspan="4" align="right">
                    ...
                    &nbsp;
                    <asp:Button CssClass="formButton" ID="btnFilterEmployees" runat="server" Text="Apply Filter" OnClick="btnFilterEmployees_Click" />
                    <asp:Button CssClass="formButton" ID="btnClearEmployeeFilters" runat="server" Text="Clear Filter" OnClick="btnClearEmployeeFilters_Click" />
                </td>
            </tr>
        </table>
    </div>
        ...
        <tr>
            <td>
                <table class="navTable">
                    <tr>
                        <td><asp:DropDownList runat="server" ID="ddlengineerGroups" /></td>
                        <td>
                            <asp:DropDownList runat="server" ID="ddlGroupFilters" OnSelectedIndexChanged="ddlGroupFilters_SelectedIndexChanged" AutoPostBack="true" />
                            <asp:HiddenField runat="server" ID="hdnGroupFilterId" />
                        </td>
                        ...
                        <td>
                            <asp:LinkButton ID="btnPrevious" CssClass="prevweek" runat="server" 
                                Text="<< prev" onclick="btnPrevious_Click"></asp:LinkButton>
                        </td>
                        <td>
                        ...
                        <td>
                            <asp:Button ID="btnThisWeek" runat="server" Text="This Week" 
                                CssClass="formButton" onclick="btnThisWeek_Click" />
                        </td>
                        <td>
                            <img src="images/spacer.gif" width="2" border="0" alt="" />
                        </td>
                        <td>
                            <asp:LinkButton ID="btnNext" CssClass="nextweek" runat="server" Text="next >>" 
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('.hoursGrid').hourbox2({
                editable: false,
                sortable: true,
                sortType: ['string','link', 'date', 'string', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer', 'integer'],
                colresizable: true,
...
//                "aaSorting": []
            //            });
                ...
                $('select[id *= "ddlengineerGroups"]').change(function () { LoadEngineerGroup($(this).val()); });
            });

            function LoadEngineerGroup(group) {
                if (group == "") {
                    return;
                }
                ...
                if (group === "Selected") {
                    var selected = $('input:checked[id *= "selectEmp"]');

                    for (var i = 0; i < selected.length; i++) {
                        ids += $(selected[i]).parent().attr('empid') + "|";
                    }
                }
                else if (group === "First5") {
                    ids = GetTopNEmpIds(5);
                }
                else if (group === "First10") {
                    ids = GetTopNEmpIds(10);
                }
                else {
                    qstr += "GID=" + group;
                    display = true;
                ...
                if (display) {
                    window.open('DisplayGroup.aspx' + qstr);
                }

                $('select[id *= "ddlengineerGroups"]').val('');
            }

            function GetTopNEmpIds(n) {
                var rows = $('td[empid]', '.scrollTable tr:lt(' + n + ')');
                var ids = "";

                for (var i = 0; i < rows.length; i++) {
                    ids += $(rows[i]).attr('empid') + "|";
                }
